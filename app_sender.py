...
# -------------------------------------------------------------------------
# 💡 設定完了: あなたの GAS Web App URL をコードに固定しました
# -------------------------------------------------------------------------
# !! この YOUR_GAS_WEB_APP_URL には、あなたの本物のURLが入っていることを確認してください !!
...
st.success("✅ Web App URLは設定済みです。このアプリのURLは今後変わりません。")


# 💡 GAS URL入力欄は表示せず、コード内の固定URLをそのまま利用します。
GAS_WEB_APP_URL = YOUR_GAS_WEB_APP_URL.strip()


# シートID入力欄
SPREADSHEET_ID_INPUT = st.text_input("🎯 ターゲットスプレッドシートID (コピーしたシートのURL全体を貼り付け可)", "")
st.info(f"書き込み先シート: {TARGET_SHEET_NAME}")


# GAS URLは設定済みとみなし、シートIDが入力されていれば実行ブロックへ
if SPREADSHEET_ID_INPUT:
    ...
    # シートIDが入力されていない場合にのみ注意書きを表示
    st.warning("⚠️ **ターゲットスプレッドシートIDを貼り付けてください。**")
